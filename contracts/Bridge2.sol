...
contract Bridge2 is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;


    uint256 public deposits;
    uint256 public minDeposit = 50000000000000; // 0.000050BTC $10 in btc at current rate
    ...
    function depositToBridge() payable public nonReentrant {
        require(msg.value >= minDeposit, 'bad rate');
        deposits = deposits + msg.value;
        depositMaps[lastId].deposit = msg.value;
        depositMaps[lastId].depositor = msg.sender; 
        lastId = lastId+1; 
        emit Deposit(msg.sender, msg.value, block.timestamp);
    }

    function withdrawFromBridge(address user, uint256 amount) public {
        require(msg.sender == bridgeOperator, 'bad call');
        (bool sent, ) = user.call{value: amount}("");
        require(sent, "Failed to send Ether");
        withdrawMaps[lastId].deposit = amount; 
        withdrawMaps[lastId].depositor = user;
        depositorsId = depositorsId+1;
        emit Withdraw(user, amount, block.timestamp);
    }

    function withdrawFromBridgeMultiple(address[] memory user, uint256[] memory amount) public {
        require(msg.sender == bridgeOperator, 'bad call');
        require(user.length == amount.length, 'user amounts mismatch');

        for(uint i=0; i<amount.length; i++){
            (bool sent, ) = user[i].call{value: amount[i]}("");
            require(sent, "Failed to send Ether");
            withdrawMaps[lastId].deposit = amount[i]; 
            withdrawMaps[lastId].depositor = user[i];
            depositorsId = depositorsId+1;
            emit Withdraw(user[i], amount[i], block.timestamp);
        }
    ...
    function withdraw(uint256 amount) public onlyOwner {
        ...
        require(amount2 > 0, "Nothing to withdraw; contract balance empty");
        require(amount <= amount2, 'Too large withdraw');
